...
class Orchestrator:
    ...
    async def _route_output(
        ...
        _outs = getattr(output, "outputs", None) or []
        _mm = getattr(output, "multimodal_output", None)
        _first = _outs[0] if _outs else None
        ...
        if (
            ...
            if (
                ...
                if req_state.streaming.enabled and finished and not final_only_finished:
    ...
    async def _forward_to_next_stage(
        ...
        *,
        ...
        logger.info(
            "[Orchestrator] _forward_to_next_stage: req=%s stage %d->%d n_inputs=%d already_submitted=%s resumable=%s",
            req_id,
            src_stage_id,
            next_logical,
            len(next_inputs) if next_inputs else 0,
            already_submitted,
